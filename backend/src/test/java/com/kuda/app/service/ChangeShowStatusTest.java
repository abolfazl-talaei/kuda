...
public class ChangeShowStatusTest {
	...
	public void setup() throws KudaException {
		...
		Mockito.when(settingService.get(SAMPLE_USERNAME, SettingKey.FEEDBACK_EFFECT))
				.thenReturn(Optional
						.of(Settings.builder().key(SettingKey.FEEDBACK_EFFECT.getName()).value("false").build()));
		...
		Mockito.when(kudaRepository.findAllDurationKudas(organization.getId(), false))
				.thenReturn(Arrays.asList(
						TestInstanceGenerator.getKudaSample(organization, user, request, DURATION),
						TestInstanceGenerator.getKudaSample(organization, user, request, DURATION),
						TestInstanceGenerator.getKudaSample(organization, user, request, DURATION)));
	...
	public void ok() throws KudaException {
		...
		try {
			kudaService.changeShowStatus(SAMPLE_USERNAME, true);
		} catch (Exception ex) {
			assertTrue(false); // fail if we entered the catch
		}
	}
}
